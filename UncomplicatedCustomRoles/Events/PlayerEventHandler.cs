...
namespace UncomplicatedCustomRoles.Events
{
    internal class PlayerEventHandler : EventHandlerBase
    {
        ...
        {
            PlayerHandler.ActivatingGenerator += OnGenerator;
            PlayerHandler.Dying += OnDying;
            PlayerHandler.Died += OnDied;
            PlayerHandler.SpawningRagdoll += OnRagdollSpawn;
                ...
                else
                {
                    ...
                    if (int.TryParse(NewRole.Value.ToString(), out int id) && CustomRole.TryGet(id, out ICustomRole role))
                    {
                        ...
                        if (summoned.TryGetModule(out KeepInventoryOnEscape module))
                            RespawnInventoryQueue.TryAdd(Escaping.Player.Id, new(new(Escaping.Player.Items.Select(i => i.Type)), Escaping.Player.Ammo, module.DropItems));
